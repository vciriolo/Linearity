...
{
  //  std::cout << " ScaleCorrector::SetRunRangeMap " << inFileName << std::endl;
  ...
  while(1)
  {
    ...
    getline(ss,runMin,'-');
    getline(ss,runMax,'-');
/*     std::cout << " atoi(runMin.c_str() =  " << atoi(runMin.c_str()) << std::endl; */
/*     std::cout << " atoi(runMax.c_str() =  " << atoi(runMax.c_str()) << std::endl; */
					 ...
					 const std::string& label, const std::string& version, const float& timesError)
{
  ...
  {
    //    std::cout << " (mapIt->second).first " << ((mapIt->second).first) << std::endl;
...
/*     std::cout << " valore = " << (*ShervinRunDepScaleMap)[dummy2] << std::endl; */

//g++ -Wall -o ApplyDiffNonLinearity_Hgg_testExtrapolation `root-config --cflags --glibs` ApplyDiffNonLinearity_Hgg_testExtrapolation.cpp
  ...
  */
  ...
  for(int iTrial = 0; iTrial < nTrials; ++iTrial)
  {
	...
	for(int nBin = 1; nBin<envelop_cat0->GetNbinsX(); ++nBin) {
	  if(envelop_cat0->GetBinCenter(nBin) <= 110.)

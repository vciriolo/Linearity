//g++ -Wall -o ApplyDiffNonLinearity_Hgg_OnlyStatPropagation `root-config --cflags --glibs` ApplyDiffNonLinearity_Hgg_OnlyStatPropagation.cpp
...
int main()
{
  ...
  bool splitLastBin = true;
  if(splitLastBin) TF1_folder = "../TF1_"+methodName+"_EtScale_splittedBin";
  std::cout << " >>>> splitLastBin = " << splitLastBin << std::endl;
  //////////////// da sotto qui ok 
  ...
  if(splitLastBin){
    ...
    corMatrix[0] = new double[nPar*nPar];
    (corMatrix[0])[0] = 1;
    (corMatrix[0])[1] = -0.20399;
    (corMatrix[0])[2] = -0.20399;
    (corMatrix[0])[3] = 1;
    corMatrix[1] = new double[nPar*nPar];
    (corMatrix[1])[0] = 1.0000;
    (corMatrix[1])[1] = 0.2469;
    (corMatrix[1])[2] = 0.2469;
    (corMatrix[1])[3] = 1.0000;
    corMatrix[2] = new double[nPar*nPar];
    (corMatrix[2])[0] = 1.0000;
    (corMatrix[2])[1] = -0.2148;
    (corMatrix[2])[2] = -0.2148;
    (corMatrix[2])[3] = 1.0000;
    corMatrix[3] = new double[nPar*nPar];
    (corMatrix[3])[0] = 1.0000;
    (corMatrix[3])[1] = -0.3760;
    (corMatrix[3])[2] = -0.3760;
    (corMatrix[3])[3] = 1.0000;
  }

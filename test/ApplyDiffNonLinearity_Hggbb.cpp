//g++ -Wall -o ApplyDiffNonLinearity_Hggbb `root-config --cflags --glibs` ApplyDiffNonLinearity_Hggbb.cpp
...
int main()
{
  ...
  //systematic uncertainties opropagated  
  ...
  // END systematic uncertainties opropagated  
  ...
  if(HggCatType == "CiC_8TeV"){
    inFile[0] = new TFile("/tmp/amartell/RadionToHH_2Gamma_2b.root", "read");
    //inFile[0] = new TFile("/tmp/amartell/Radion_nm.root", "read");
...
//     inFile[3] = new TFile("../data/histograms_CMS-HGG.root", "read");
    trees[0] = (TTree*)inFile[0]->Get("Radion_m300_8TeV");
    //trees[0] = (TTree*)inFile[0]->Get("Radion_m1000_8TeV_nm");
...
//     trees[3] = (TTree*)inFile[3]->Get("tth_m125_8TeV"); 
  ...
  for(int iSample = 0; iSample < nSamples; ++iSample)
  {
    ...
    trees[iSample]->SetBranchStatus("evweight", 1);    trees[iSample]->SetBranchAddress("evweight", &full_weight);
...
//     trees[iSample]->SetBranchStatus("dipho_mva", 1);       trees[iSample]->SetBranchAddress("dipho_mva", &dipho_mva);
    ...
    trees[iSample]->SetBranchStatus("PhotonsMass", 1);           trees[iSample]->SetBranchAddress("PhotonsMass", &mass);
    trees[iSample]->SetBranchStatus("ph1_pt", 1);             trees[iSample]->SetBranchAddress("ph1_pt", &et1);
    trees[iSample]->SetBranchStatus("ph2_pt", 1);             trees[iSample]->SetBranchAddress("ph2_pt", &et2);
    trees[iSample]->SetBranchStatus("ph1_eta", 1);           trees[iSample]->SetBranchAddress("ph1_eta", &eta1);
    trees[iSample]->SetBranchStatus("ph2_eta", 1);           trees[iSample]->SetBranchAddress("ph2_eta", &eta2);
    trees[iSample]->SetBranchStatus("ph1_r9", 1);           trees[iSample]->SetBranchAddress("ph1_r9", &r91);
    trees[iSample]->SetBranchStatus("ph2_r9", 1);           trees[iSample]->SetBranchAddress("ph2_r9", &r92);
  }
  ...
  {
    inFile[iSample]->cd();
    std::cout << " trees[iSample]->GetEntries() = " << trees[iSample]->GetEntries() << std::endl;
    for(int entry = 0; entry < trees[iSample]->GetEntries(); ++entry)
    {
      ...
      //      if( HggCatType == "CiC_7TeV" ) eventMVAcat = full_cat;
      if( HggCatType == "CiC_8TeV") eventMVAcat = 0;
...
// 	  std::cout << " full_weight = " << full_weight << std::endl;
...
Hgg_measuredDiff_stat[iTrial*nCats+iCat]->Fill(mass*sqrt(f_stat_Trial_ET[pho1Cat]->Eval(et1) * f_stat_Trial_ET[pho2Cat]->Eval(et2)), full_weight);
  ...
  TFile outputFile("rescaledHggbb_histos.root","RECREATE");
  outputFile.cd();
  if(HggCatType == "MVA_8TeV") nCats = 5;
  for(int iCat = 0; iCat < nCats; ++iCat)
  {
    std::cout << " stampo iCat = " << iCat << std::endl;
    SingleCat[iCat]->Write();
    MassCorrection[iCat]->Write();
    ETSinglePho[iCat]->Write();
    HTDoublePho[iCat]->Write();

    std::cout << " stampo Hgg_original " << iCat << std::endl;

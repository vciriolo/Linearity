//g++ -Wall -o ApplyDiffNonLinearity_Hgg_testExtrapolation_7TeV `root-config --cflags --glibs` ApplyDiffNonLinearity_Hgg_testExtrapolation_7TeV.cpp
...
// 	  std::cout << " corr = " << sqrt(f_std[pho1Cat]->Eval(et1) * f_std[pho2Cat]->Eval(et2)) << std::endl;
  ...
  TF1** f_stat_Trial_inflated = new TF1*[nCats];
  for(int iCat = 0; iCat < nCats; ++iCat)
    {
      ...
      f_stat_Trial_inflated[iCat] = new TF1(Form("f_stat_Trial_inflated_cat%d_std",iCat), "[0]+[1]*(x-90.)",20., 1000.);
      f_stat_Trial_inflated[iCat]->SetParameter(0, f_stat[iCat]->GetParameter(0));
      f_stat_Trial_inflated[iCat]->SetParameter(1, f_stat[iCat]->GetParameter(1));
      f_stat_Trial_inflated[iCat]->SetParError(0, f_stat[iCat]->GetParError(0));
      f_stat_Trial_inflated[iCat]->SetParError(1, f_stat[iCat]->GetParError(1));
    }
  TF1** f_stat_Trial_ET = new TF1*[nCats];
  TF1** f_stat_Trial_ET_inflated = new TF1*[nCats];
  for(int iCat = 0; iCat < nCats; ++iCat)
    {
      ...
      f_stat_Trial_ET_inflated[iCat] = new TF1(Form("f_stat_Trial_ET_inflated_cat%d_std",iCat), "[0]+ [1] * (x-45.)",20., 1000.);
    }
  ...
  for(int iTrial = 0; iTrial < nTrials; ++iTrial)
  {
    ...
    for(int iCat = 0; iCat < nCats; ++iCat)
    {
      ...
      double* parRand_inflated = new double[nPar];
      double* fiParRndm = new double[nPar];
      double* fiParRndm_inflated = new double[nPar];
      for(int iPar = 0; iPar < nPar; ++iPar) {
	fiParRndm[iPar] = gRandom->Gaus(0.,1.);
	//	fiParRndm_inflated[iPar] = 2.*fiParRndm[iPar];
	fiParRndm_inflated[iPar] = fiParRndm[iPar];
      }      
      //std::cout << ">>> iCat: " << iCat << std::endl;
      for(int j = 0; j < nPar; ++j)
      {
        double tmp = 0;
	double tmp_inflated = 0;
	for(int i = 0; i < nPar; ++i)
        {
          tmp += fiParRndm[i] * (C[iCat])[i+j*nPar];
	  tmp_inflated += fiParRndm_inflated[i] * (C[iCat])[i+j*nPar];
        }
        tmp *= (initParErr[iCat])[j];
        parRand[j] = (initPar[iCat])[j] + tmp;

	tmp_inflated *= (initParErr[iCat])[j];
	parRand_inflated[j] = (initPar[iCat])[j] + tmp_inflated;
	//std::cout << ">>>>>> iPar: " << j << "   ratio/err: " << fabs(parRand[j]-(initPar[iCat])[j])/(initParErr[iCat])[j] << std::endl;
      ...
      f_stat_Trial_inflated[iCat] -> SetParameters( parRand_inflated );


      delete parRand;
      delete fiParRndm;
      delete parRand_inflated;
      delete fiParRndm_inflated;
      ...
      f_stat_Trial_ET_inflated[iCat]->SetParameter(0, f_stat_Trial_inflated[iCat]->GetParameter(0));
      f_stat_Trial_ET_inflated[iCat]->SetParError(0, f_stat_Trial_inflated[iCat]->GetParError(0));
      f_stat_Trial_ET_inflated[iCat]->SetParameter(1, 2.*f_stat_Trial_inflated[iCat]->GetParameter(1));
      f_stat_Trial_ET_inflated[iCat]->SetParError(1, 2.*f_stat_Trial_inflated[iCat]->GetParError(1));

      if(iCat == 0)
	for(int nBin = 1; nBin<envelop_cat0->GetNbinsX(); ++nBin) {
	  if(envelop_cat0->GetBinCenter(nBin) <= 100.)
	    envelop_cat0->Fill(envelop_cat0->GetBinCenter(nBin), f_stat_Trial_ET[iCat]->Eval(envelop_cat0->GetBinCenter(nBin)));
	  else  envelop_cat0->Fill(envelop_cat0->GetBinCenter(nBin), f_stat_Trial_ET_inflated[iCat]->Eval(envelop_cat0->GetBinCenter(nBin)));
	}
      if(iCat == 1)
	for(int nBin = 1; nBin<envelop_cat1->GetNbinsX(); ++nBin) {
	  ...
	  else  envelop_cat1->Fill(envelop_cat1->GetBinCenter(nBin), f_stat_Trial_ET_inflated[iCat]->Eval(envelop_cat1->GetBinCenter(nBin)));
	}
      if(iCat == 2)
        for(int nBin = 1; nBin<envelop_cat2->GetNbinsX(); ++nBin) {
          ...
          else  envelop_cat2->Fill(envelop_cat2->GetBinCenter(nBin), f_stat_Trial_ET_inflated[iCat]->Eval(envelop_cat2->GetBinCenter(nBin)));
        }
      if(iCat == 3)
	    ...
	    envelop_cat3->Fill(envelop_cat3->GetBinCenter(nBin), f_stat_Trial_ET[iCat]->Eval(envelop_cat3->GetBinCenter(nBin)));
          else  envelop_cat3->Fill(envelop_cat3->GetBinCenter(nBin), f_stat_Trial_ET_inflated[iCat]->Eval(envelop_cat3->GetBinCenter(nBin)));
        }
...
//        }
	...
	if(HggCatType == "MVA_8TeV") nCats = 5;
	    ...
	    else if(et1 <= 110. && et2 > 110. ) {
	      Hgg_measuredDiff_stat[iTrial*nCats+iCat]->Fill(mass*sqrt(f_stat_Trial_ET[pho1Cat]->Eval(et1) *
								       f_stat_Trial_ET_inflated[pho2Cat]->Eval(et2)), full_weight);
	      if( (dipho_pt/mass) < (40./125.) )
		Hgg_measuredDiff_stat_pTLow[iTrial*nCats+iCat]->Fill(mass*sqrt(f_stat_Trial_ET[pho1Cat]->Eval(et1) *
									       f_stat_Trial_ET_inflated[pho2Cat]->Eval(et2)), full_weight);
	      else
	    ...
	    else if(et2 <= 110. && et1 > 110.) {
	      Hgg_measuredDiff_stat[iTrial*nCats+iCat]->Fill(mass*sqrt(f_stat_Trial_ET_inflated[pho1Cat]->Eval(et1) *
								       f_stat_Trial_ET[pho2Cat]->Eval(et2)), full_weight);
	      if( (dipho_pt/mass) < (40./125.) )
		Hgg_measuredDiff_stat_pTLow[iTrial*nCats+iCat]->Fill(mass*sqrt(f_stat_Trial_ET_inflated[pho1Cat]->Eval(et1) *
									       f_stat_Trial_ET[pho2Cat]->Eval(et2)), full_weight);
	      else
		Hgg_measuredDiff_stat_pTHigh[iTrial*nCats+iCat]->Fill(mass*sqrt(f_stat_Trial_ET_inflated[pho1Cat]->Eval(et1) *
										f_stat_Trial_ET[pho2Cat]->Eval(et2)), full_weight);
	    }
	    else if(et2 > 110. && et1 > 110.) {
	      Hgg_measuredDiff_stat[iTrial*nCats+iCat]->Fill(mass*sqrt(f_stat_Trial_ET_inflated[pho1Cat]->Eval(et1) *
	    ...
	    else {std::cout << " >>> che cavolo di ET sei " << std::endl;}
          }
        }
	nCats = 4;
      }
